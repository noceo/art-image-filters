...
def create_color_palette(img, k, max_size=200, n_init=10):
  ...
  color_shifts = [(0,50,0), (20,20,0), (-20,20,0)]
...
def create_gradient_field(img):
  ...
  s = 2*radius+1
  x = cv2.Scharr(grayscale_img, cv2.CV_32F, 1, 0)
  y = cv2.Scharr(grayscale_img, cv2.CV_32F, 0, 1)
  x = cv2.GaussianBlur(x, (s,s), 0)
  y = cv2.GaussianBlur(y, (s,s), 0)
  return x, y

def generateStrokeImage(img):
  palette = create_color_palette(img, 20)
  ...
  for i, (y,x) in tqdm(enumerate(grid)):
...
if __name__ == '__main__':
  ...
  output = generateStrokeImage(img)

  # # Writing the result
  if args.outfile:
    plt.imsave(f"{outputDir}{args.outfile}.jpg", output)
  else:
    cv2.imwrite("{}stroke_based_img_{}.jpg".format(outputDir, str(index).zfill(2)), output)